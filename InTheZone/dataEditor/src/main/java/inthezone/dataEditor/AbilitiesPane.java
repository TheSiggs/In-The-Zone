...
public class AbilitiesPane extends VBox {
	...
	private final SimpleBooleanProperty isCharacterLoaded =
		new SimpleBooleanProperty(false);
	private TreeItem<AbilityInfoModel> tableRoot;

	public void setAbilities(
		final ObservableValue<String> name, final TreeItem<AbilityInfoModel> abilities
	) {
	...
	public AbilitiesPane(
		...
		addMana.disableProperty().bind(
			...
			.or(new BooleanBinding() {
				...
				protected boolean computeValue() {
					...
					final AbilityInfoModel v = item.getValue();
					final AbilityInfoModel pv = item.getParent().getValue();
		...
		add.setOnAction(event -> {
			if (isCharacterLoaded.getValue()) {
				TreeItem<AbilityInfoModel> item =
					new TreeItem<>(new AbilityInfoModel(false, false));
				final ObservableList<TreeItem<AbilityInfoModel>> children = tableRoot.getChildren();
				children.add(item);
		...
		remove.setOnAction(event -> {
			final Alert m = new Alert(Alert.AlertType.CONFIRMATION, null,
			...
			m.showAndWait()
				...
				.ifPresent(response -> {
					selected.stream()
						...
						.forEach(i -> {
							final TreeItem<AbilityInfoModel> item = table.getTreeItem(i);
		...
		addSubsequent.setOnAction(event -> {
			if (selected.size() >= 1) {
				...
				final List<TreeItem<AbilityInfoModel>> all = parent.getChildren();
				int i;
				...
				final TreeItem<AbilityInfoModel> newSubsequent =
		...
		up.setOnAction(event -> {
			if (selected.size() >= 1) {
				...
				final AbilityInfoModel v = item.getValue();
				if (!v.getIsMana()) {
					final List<TreeItem<AbilityInfoModel>> all = item.getParent().getChildren();
	...
	private <S, T> void hookOnEditCommit(
		...
		final EventHandler<TreeTableColumn.CellEditEvent<S,T>> oldHandler = column.getOnEditCommit();

		final TreeItem<AbilityInfoModel> i = table.getSelectionModel().getSelectedItem();
		if (i != null) {
			try {
				description.setText((
	...
	static AbilityInfo encodeAbility(
		final TreeItem<AbilityInfoModel> item, final boolean isMana
	) throws CorruptDataException
	{
		try {
			Optional<AbilityInfo> mana;
			if (isMana) mana = Optional.empty(); else {
				final Optional<TreeItem<AbilityInfoModel>> tm = item.getChildren().stream()
					.filter(i -> i.getValue().getIsMana()).findAny();
				mana = tm.isPresent()? Optional.of(encodeAbility(tm.get(), true)) : Optional.empty();
			}
	...
	static AbilityInfo encodeAbility0(
		final AbilityInfoModel a,
		final Optional<AbilityInfo> mana,
		final Optional<AbilityInfo> subsequent,
		final boolean isMana, final boolean isSubsequent
	) throws CorruptDataException
	{
		...
		} catch (final CorruptDataException e) { /* IGNORE */ }
		try {
