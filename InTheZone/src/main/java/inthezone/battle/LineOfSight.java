...
public class LineOfSight {
	 ...
	 * character
	 * */
	public static List<MapPoint> getLOS(
	 ...
	 * diamond
	 * */
	public static List<MapPoint> getDiamond(MapPoint p0, int size) {
		List<MapPoint> r = new ArrayList<MapPoint>();
		for (int y = -size; y <= size; y++) {
			int l = size - Math.abs(y);
			for (int x = -l; x <= l; x++) {
				r.add(p0.add(new MapPoint(x, y)));
			}
		}
		return r;
	}
	...
	private static List<MapPoint> getLOS0(
		MapPoint p0,   // start position
		MapPoint p1,   // target position
		MapPoint dp,   // primary displacement
		MapPoint ds,   // secondary displacement
		double dsdp,   // gradient of the line
		boolean bias   // true to prefer the primary direction
	) {
		final List<MapPoint> r = new ArrayList<MapPoint>();
