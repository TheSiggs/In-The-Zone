...
public class CharacterPane extends TitledPane {
	...
	public Collection<AbilityInfo> getAbilities() throws CorruptDataException {
		try {
			final Collection<AbilityInfo> r = new ArrayList<>();
			for (final TreeItem<AbilityInfoModel> tm : abilitiesRoot.getChildren()) {
				r.add(AbilitiesPane.encodeAbility(tm, false));
			}
			return r;
	...
	public CharacterPane(
		...
		for (final AbilityInfo i : character.abilities) {
		...
		for (final SpriteInfo i : gameData.getGlobalSprites()) {
