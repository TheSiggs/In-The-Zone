...
public class BattleState {
	...
	public void notifyTurn() {
		turn += 1;
		for (final Zone z : zones) z.notifyTurn();
	}
	...
	public List<Zone> removeExpiredZones() {
		...
		for (final Zone z : zones) {
			if (z.reap()) {
				r.add(z);
				for (final MapPoint p : z.range) zoneMap.remove(p);
			}
		}
	...
	public int pathCost(final List<MapPoint> path) {
		...
		for (final MapPoint p : path.subList(1, path.size())) {
	...
	public Collection<Targetable> getAbilityTargets(
		...
		final Optional<List<Targetable>> r =
			getAgentAt(agent, agentType).map(a ->
				area.stream()
					.flatMap(p -> getTargetableAt(p).stream())
					.filter(t -> ability.canTarget(a, t) && ability.canAffect(t))
					.collect(Collectors.toList()));
		return r.orElse(new ArrayList<>());
	}
